...
public:
    ...
    bool init_by_path(const std::string &vertex_shader_path,
    ...
    { 
        glUniform1i(glGetUniformLocation(program_shader, name.c_str()), value); 
    }
    void setMat4(const std::string &name, const glm::mat4 &mat) const
    {
        glUniformMatrix4fv(glGetUniformLocation(program_shader, name.c_str()), 1, GL_FALSE, &mat[0][0]);
    }
    void setVec3(const std::string &name, const glm::vec3 &value) const
    { 
        glUniform3fv(glGetUniformLocation(program_shader, name.c_str()), 1, &value[0]); 
    }
    void setVec3(const std::string &name, float x, float y, float z) const
    { 
        glUniform3f(glGetUniformLocation(program_shader, name.c_str()), x, y, z); 
    }
};
