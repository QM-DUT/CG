...
void ElementBufferObject::update(const Eigen::MatrixXi& M)
{
	assert(id != 0);
	//这里bind的了也是可以的
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, id);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(int)*M.size(), M.data(), GL_DYNAMIC_DRAW);
	rows = M.rows();
	cols = M.cols();
	check_gl_error();
}
...
bool Program::init_by_path(
  const std::string &vertex_shader_path,
  const std::string &fragment_shader_path,
  const std::string &fragment_data_name)
{
	string vertex_shader_string,fragment_shader_string;
	ifstream vertex_shader_file(vertex_shader_path.c_str());
	ifstream fragment_shader_file(fragment_shader_path.c_str());
	string line;
	...
	while (getline (vertex_shader_file, line)) // line中不包括每行的换行符
    { 
	...
	while (getline (fragment_shader_file, line)) // line中不包括每行的换行符
    { 
...
GLuint Program::create_shader_helper(GLint type, const std::string &shader_string)
{
  ...
  //利用shader source真正创建shader
  //第二个参数string包含的字符串个数。我们只用了一个字符串表示着色器源代码，因此传入1。
  //第三个参数，字符串地址
  //第四个参数代表结束符
